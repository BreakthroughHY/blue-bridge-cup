...
// 思路没有毛病，不过空间复杂度和时间复杂度都很高n^2
vector<vector<string>> SolutionAlphabeticHeterotopicWordGrouping::groupAnagrams(vector<string>& strs)
{
...
// 看个大致模仿官方方法   写完对比理论上效率比官方低，空间上优
vector<vector<string>> SolutionAlphabeticHeterotopicWordGrouping::groupAnagramsLeetcode(vector<string>& strs)
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		uMap[temp].emplace_back(i);
	}

	vector<string> tempStr;
	for (auto& a : uMap)
	{
		for (auto strIndex : a.second)
		{
			tempStr.emplace_back(strs[strIndex]);
		}
...
// 完全的官方方法     效果并不理想将数据写入容器消耗大量的时间和空间   导致两个指标都不理性
vector<vector<string>> SolutionAlphabeticHeterotopicWordGrouping::groupAnagramsLeetCode(vector<string>& strs)
{
	vector<vector<string>> ans;
